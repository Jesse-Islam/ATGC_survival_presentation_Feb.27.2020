...
* Want to easily model non-proportional hazards. [2]
...
* SUPPORT study
* Casebase sampling
* Penalized logistic regression on survival data
* Maximum likelihood with regularization
* Absolute risk comparison
* Future work
* References

# SUPPORT dataset (knaus)
...
# SUPPORT manual imputation (knaus)
...
# Removed variables (knaus)

* Hospital Charges.
* Patient ratio of costs to charges.
* Patient Micro-costs.
* functional disability (ordinal).
* Income (ordinal).
* Surrogate activities of daily living.

# Variable overview (knaus)
...
# Original SUPPORT analysis (knaus)
...
ggplot(as.data.frame(baseCoxComparisonData), aes(Time)) + 
  ...
  labs(y= "Probability of Event", x = "Survival time (Days)")
```
...
* Linear associations without physiology scores perform similarly to physiology scores alone.
...
# Maximum log-likelihood [1]
 ...
 $$  log(l(\beta))=\sum^{N}_{i=1}(\sum^{K}_{k=0}x_{ik}\beta_{k})-n_{i}log(1+e^{\sum^{K}_{k=0}x_{ik}\beta_{k}})  $$
 
# Maximum log-likelihood, with offset

 $$  log(l(\beta))=\sum^{N}_{i=1}(\sum^{K}_{k=0}x_{ik}\beta_{k}+\textcolor{blue}{log(\frac{b}{B})})-n_{i}log(1+e^{\sum^{K}_{k=0}x_{ik}\beta_{k}+\textcolor{blue}{log(\frac{b}{B})}})$$


# Maximum log-likelihood, with offset and lasso

 $$  log(l(\beta))=\sum^{N}_{i=1}(\sum^{K}_{k=0}x_{ik}\beta_{k}+\textcolor{blue}{log(\frac{b}{B})})-n_{i}log(1+e^{\sum^{K}_{k=0}x_{ik}\beta_{k}+\textcolor{blue}{log(\frac{b}{B})}})  \textcolor{red}{-\lambda||\beta||}$$
...
abCbFull=casebase::absoluteRisk(cbFull,time = seq(1,max(abKMcompare[,1]), 30),newdata = newData , s="lambda.1se",method=c("numerical"))
