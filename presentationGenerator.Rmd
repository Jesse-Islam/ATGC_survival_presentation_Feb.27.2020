...
* Linear associations without physiology scores perform similarly to physiology scores.
...
# Maximum log-likelihood [1]
 ...
 $$  l(\beta)=\sum^{N}_{i=1}(\sum^{K}_{k=0}x_{ik}\beta_{k})-n_{i}log(1+e^{\sum^{K}_{k=0}x_{ik}\beta_{k}})  $$
 
# Maximum log-likelihood, with offset

 $$  l(\beta)=\sum^{N}_{i=1}(\sum^{K}_{k=0}x_{ik}\beta_{k}+\textcolor{blue}{\frac{b}{B}})-n_{i}log(1+e^{\sum^{K}_{k=0}x_{ik}\beta_{k}+\textcolor{blue}{\frac{b}{B}}})$$


# Maximum log-likelihood, with offset and lasso

 $$  l(\beta)=\sum^{N}_{i=1}(\sum^{K}_{k=0}x_{ik}\beta_{k}+\textcolor{blue}{\frac{b}{B}})-n_{i}log(1+e^{\sum^{K}_{k=0}x_{ik}\beta_{k}+\textcolor{blue}{\frac{b}{B}}})  \textcolor{red}{-\lambda||\beta||}$$
...
* (death, time) ~ 1
* (death, time) ~  $\beta X$
* (death, time) ~  $\beta X$ <--Lasso
* death ~ log(time) + $\beta X$ <--Lasso 
