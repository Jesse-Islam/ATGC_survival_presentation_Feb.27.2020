...
* Participants: 9105 hospitalized s
...
1. Compare SPS and APS over ~5.56 years using absolute risk curves.
2. Compare to Kaplan-Meier curve
3. Compare to full model (excluding SPS and APS)
...
coxNaive<- survival::coxph(Surv(time=V1,event=V2) ~ .-sps -aps, data = train)
abcoxNaive<-survival::survfit(coxAPS,newdata = test)

KM<- survival::coxph(Surv(time=V1,event=V2) ~ 1, data = test)
abKM<-survival::survfit(KM)
abKMcompare<-cbind(abKM$time[abKMcompare[,1]<=1631],1-abKM$surv[abKMcompare[,1]<=1631])
colnames(abKMcompare)<-c("Time","KM")

baseCoxComparisonData<-cbind(c(1:1631),1-rowMeans(abcoxSPS$surv),1-rowMeans(abcoxAPS$surv),1-abKM$surv,1-rowMeans(abcoxNaive$surv))
colnames(baseCoxComparisonData)<-c("Time","SPS","APS","KM","Naive")
```
...
ggplot(as.data.frame(baseCoxComparisonData), aes(Time)) + 
  ...
  geom_step(data=as.data.frame(abKMcompare),mapping=aes(x=Time,y=KM,color="KM"),lwd=2)
#ZOOM
ggplot(as.data.frame(baseCoxComparisonData), aes(Time)) + 
  geom_line(aes(y = SPS, colour = "SPS"),lwd=1) + 
  geom_line(aes(y = APS, colour = "APS"),lwd=1) +
  geom_step(data=as.data.frame(abKMcompare),mapping=aes(x=Time,y=KM,color="KM"))+
 scale_x_continuous(limits = c(1, 500))

  geom_line(aes(y = Naive, colour = "Naive"),lwd=1)ggplot(as.data.frame(baseCoxComparisonData[c(150:500),]), aes(Time)) + 
  geom_line(aes(y = SPS, colour = "SPS"),lwd=1) + 
  geom_line(aes(y = APS, colour = "APS"),lwd=1) +
  geom_line(aes(y = KM, colour = "KM"),lwd=1) 

ggplot(as.data.frame(baseCoxComparisonData[c(150:500),]), aes(Time)) + 
  geom_line(aes(y = SPS, colour = "SPS"),lwd=1) + 
  geom_line(aes(y = KM, colour = "KM"),lwd=1) +
  geom_line(aes(y = Naive, colour = "Naive"),lwd=1)
...
cbNaive=casebase::fitSmoothHazard.fit(x[train_index,],y[train_index,],family="glmnet",time="d.time",event="death",formula_time = ~log(d.time),alpha=1,ratio=ratio)
```
...
abCbNaive=casebase::absoluteRisk(cbNaive,time = seq(1,max(completeData$d.time), 20),newdata = x[test_index,] , s="lambda.1se",method=c("numerical"))
